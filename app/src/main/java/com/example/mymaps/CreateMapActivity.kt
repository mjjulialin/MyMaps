...
supportActionBar?.title = intent.getStringExtra(EXTRA_MAP_TITLE)
        ...
        mapFragment.view?.let {
            Snackbar.make(it, "Long press to add a marker! Press X to clear the markers!", Snackbar.LENGTH_INDEFINITE)
                .setAction("OK", {})
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        if(item.itemId == R.id.miSave) {
            ...
            mMap.clear()
            markers.clear()
            val userMap = intent.getStringExtra("EXTRA_MAP_TITLE")?.let { UserMap(it, emptyList()) }
            val data = Intent()
            data.putExtra(EXTRA_USER_MAP, userMap)
            setResult(Activity.RESULT_OK, data)
            return true
    ...
    private fun showAlertDialog(latLng: LatLng) {
        ...
        dialog.getButton(DialogInterface.BUTTON_POSITIVE).setOnClickListener {
            ...
            if (title.trim().isEmpty() || description.trim().isEmpty()) {
                Toast.makeText(
                    this,
                    "Pace must have non-empty title and description",
                    Toast.LENGTH_LONG
                ).show()
